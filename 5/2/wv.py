...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    corrected_e_w1B = np.sqrt(np.absolute(1-np.sum(np.square(mu_orth)))) * ((e_w1B - mu_B)/(np.linalg.norm((e_w1B - mu_B))))
    corrected_e_w2B = np.sqrt(np.absolute(1-np.sum(np.square(mu_orth)))) * ((e_w2B - mu_B)/(np.linalg.norm((e_w2B - mu_B))))
